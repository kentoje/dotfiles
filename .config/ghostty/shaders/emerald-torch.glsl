...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // Color temperature: bright emerald at head → deep forest green at tail
    vec4 hotColor = trail + vec4(0.0, 0.15, 0.05, 0.0);     // bright emerald at head
    vec4 coolColor = vec4(0.02, 0.4, 0.15, 1.0);            // deep forest green at tail
    ...
    // Inner core: bright emerald-white center line
    ...
    vec4 coreColor = vec4(0.85, 1.0, 0.9, 1.0); // minty white-hot
    ...
    vec4 pulseColor = trail + vec4(0.0, 0.1, 0.05, 0.0);
    ...
    vec4 smokeColor = vec4(0.01, 0.05, 0.03, 1.0); // dark emerald-tinted smoke
    ...
    for (int i = 0; i < PARTICLE_COUNT; i++) {
        ...
        // Emerald color shift: brighter/greener embers
        vec4 emberCol = trail + vec4(0.0, 0.2 * seed, 0.1 * seed, 0.0);

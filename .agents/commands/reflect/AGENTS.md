...
Review the work you just completed and capture any reusable learnings in `.learnings/`.

## Steps

1. **Read existing learnings.** Scan every file in `.learnings/` so you don't duplicate what's already documented.
...
4. **Write one file per learning** in `.learnings/` using a short kebab-case filename that includes a timestamp (e.g. `2026-02-24-1430-carbon-api-gotcha.md`). The format is `YYYY-MM-DD-HHmm-<slug>.md`. Each file should contain:
...
Write learnings to `.learnings/` in the current project root (the directory the agent session is running in). Create the directory if it doesn't exist.
